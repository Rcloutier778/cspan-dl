...
def format_string(string, width=80):
    replace = ['"',"'",'(',')']
    def do_replace(strng):
        for rpl in replace:
            strng = strng.replace(rpl, '')
        return strng
    return '\n '.join(
        [
            do_replace(line).strip()
            for line in pprint.pformat(string, width=width).splitlines()
        ]
    )
...
class SeriesButton(tk.Button):
    def __init__(self, master, seriesName, pathVar):
        ...
        fmt_name = format_string(self.seriesName, 30) #pprint.pformat(self.seriesName, width=30)
...
class DownloadPickerButton(tk.Button):
    def __init__(self, master=None, name=None, abstract=None, abstractFrame=None, max_abstract_depth=None):
        ...
        fmt_name = format_string(self.name, 30)# pprint.pformat(self.name, width=30)
        super().__init__(self.master, text=fmt_name, command=self.myCommand, font=('Arial', 15))
        self.bind("<Enter>", self.on_enter)
        self.bind("<Leave>", self.on_leave)

    def on_enter(self, event):
        formatted_abstract = format_string(self.abstract, 120) #pprint.pformat(self.abstract, width=120)
    ...
    def __init__(self, master=None):
        ...
        def on_enter(fmt):
            def on_enter_wrapper(event):
                formatted_abstract = format_string(fmt_to_hint[fmt]) #pprint.pformat(fmt_to_hint[fmt])
                self.abstractFrame.configure(text=formatted_abstract)
        ...
        self.keep_mp4 = tk.Checkbutton(self, text="Also keep the downloaded mp4 file?", font=FONT,
                                       variable=self.keep_mp4_var)
        self.keep_mp4.pack()
